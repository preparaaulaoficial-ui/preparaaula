...
// ─── PPTX — cores sem # e helpers ────────────────────────────────────────────
const PPT_CORES = {
  azul:    { primary: '1a56db', dark: '0d1f4e', light: 'dbeafe', accent: '93c5fd' },
  verde:   { primary: '059669', dark: '052e16', light: 'd1fae5', accent: '6ee7b7' },
  roxo:    { primary: '7c3aed', dark: '1e1b4b', light: 'ede9fe', accent: 'c4b5fd' },
  laranja: { primary: 'ea580c', dark: '431407', light: 'ffedd5', accent: 'fdba74' },
  rosa:    { primary: 'db2777', dark: '500724', light: 'fce7f3', accent: 'f9a8d4' },
  cinza:   { primary: '334155', dark: '0f172a', light: 'e2e8f0', accent: '94a3b8' },
}
const NAVY = '152664'
const SKY  = 'CBE7FE'
const circleColors = ['1ABFBF', 'F5C842', 'F96167', '4CAF50', '3b82f6', '8b5cf6']
const trunc = (s, n) => s && s.length > n ? s.slice(0, n - 1) + '…' : (s || '')

const ETAPAS_LABEL = {
  introducao: 'Introdução', desenvolvimento: 'Desenvolvimento',
  execucao: 'Execução', revisao: 'Revisão',
}
...
// ─── EXPORTAR PPTX (design editorial) ────────────────────────────────────────
async function exportarPPT(slides, aula, palNome = 'azul') {
  ...
  const C     = PPT_CORES[palNome] || PPT_CORES.azul
  const total = slides.length

  // helper rodapé
  const addRodape = (sl) => {
    sl.addShape(prs.shapes.RECTANGLE, { x:0, y:5.44, w:10, h:0.008, fill:{color:'e8eef8'}, line:{color:'e8eef8',width:0} })
  ...
  // helper bullets com círculos coloridos
  const addBullets = (sl, items, x, y, w) => {
    items.forEach((item, j) => {
      const cy = y + j * 0.66
      const cc = circleColors[j % circleColors.length]
      sl.addShape(prs.shapes.OVAL, { x, y: cy+0.05, w:0.21, h:0.21, fill:{color:cc}, line:{color:cc,width:0} })
      sl.addText(trunc(item, 110), {
        x: x+0.29, y: cy, w: w-0.31, h:0.56,
        fontSize:11.5, color: j===0 ? '0f172a' : '374151',
        fontFace:'Calibri', valign:'middle', wrap:true, margin:0, bold: j===0,
      })
    })
  }

  // helper header slide conteúdo
  const addHeader = (sl, etapa, num, numTotal) => {
    // faixa topo
    sl.addShape(prs.shapes.RECTANGLE, { x:0, y:0, w:10, h:0.055, fill:{color:C.primary}, line:{color:C.primary,width:0} })
    // logo
    sl.addText('Prepara Aula', { x:0.35, y:0.1, w:2.2, h:0.22, fontSize:7.5, bold:true, color:NAVY, fontFace:'Calibri', margin:0, charSpacing:0.5 })
    // tag etapa
    if (etapa) {
      sl.addShape(prs.shapes.ROUNDED_RECTANGLE, { x:7.45, y:0.09, w:1.4, h:0.23, fill:{color:C.primary,transparency:88}, line:{color:C.primary,transparency:65,width:0.5}, rectRadius:0.03 })
      sl.addText(etapa.toUpperCase(), { x:7.45, y:0.088, w:1.4, h:0.234, fontSize:6.5, bold:true, color:C.primary, fontFace:'Calibri', align:'center', valign:'middle', charSpacing:1, margin:0 })
    }
    // número
    sl.addText(`${num} / ${numTotal}`, { x:8.95, y:0.09, w:0.7, h:0.22, fontSize:7, color:'94a3b8', fontFace:'Calibri', align:'right', margin:0 })
    // linha divisória
    sl.addShape(prs.shapes.RECTANGLE, { x:0.35, y:0.43, w:9.3, h:0.008, fill:{color:'e8eef8'}, line:{color:'e8eef8',width:0} })
  }

  slides.forEach((s, idx) => {
    const sl       = prs.addSlide()
    const tipo     = s.tipo    || 'conceito'
    const etapa    = ETAPAS_LABEL[s.etapa] || ''
    ...
    if (tipo === 'capa' || idx === 0) {
      ...
      sl.addShape(prs.shapes.RECTANGLE, { x:0, y:0, w:0.06, h:5.625, fill:{color:SKY}, line:{color:SKY,width:0} })

      // Retângulo direito semitransparente
      sl.addShape(prs.shapes.RECTANGLE, { x:5.8, y:0, w:4.2, h:5.625, fill:{color:C.primary,transparency:82}, line:{color:NAVY,width:0} })

      // Círculos decorativos direita
      sl.addShape(prs.shapes.OVAL, { x:7.2, y:-1.0, w:3.2, h:3.2, fill:{color:'FFFFFF',transparency:94}, line:{color:'FFFFFF',transparency:88,width:0.5} })
      sl.addShape(prs.shapes.OVAL, { x:8.5, y:3.8,  w:1.8, h:1.8, fill:{color:'FFFFFF',transparency:92}, line:{color:'FFFFFF',width:0} })

      // Tag disciplina
      if (aula?.disciplina) {
        sl.addShape(prs.shapes.ROUNDED_RECTANGLE, { x:0.5, y:0.35, w:Math.max(1.2, aula.disciplina.length*0.095+0.4), h:0.28, fill:{color:C.primary,transparency:35}, line:{color:SKY,transparency:60,width:0.5}, rectRadius:0.04 })
        sl.addText(aula.disciplina.toUpperCase(), { x:0.5, y:0.33, w:3.5, h:0.3, fontSize:7.5, bold:true, color:SKY, charSpacing:2, fontFace:'Calibri', margin:0 })
      }

      // Título
      sl.addText(trunc(titulo, 90), { x:0.5, y:0.82, w:5.1, h:2.8, fontSize:34, bold:true, color:'FFFFFF', fontFace:'Calibri', valign:'top', wrap:true, charSpacing:-0.5 })

      // Meta (turma · duração)
      const meta = [aula?.nivel, aula?.duracao].filter(Boolean).join(' · ')
      if (meta) {
        sl.addShape(prs.shapes.RECTANGLE, { x:0.5, y:3.68, w:0.03, h:0.52, fill:{color:SKY}, line:{color:SKY,width:0} })
        sl.addText(meta, { x:0.65, y:3.65, w:4.8, h:0.58, fontSize:12, color:SKY, transparency:30, fontFace:'Calibri', valign:'middle', margin:0 })
      }

      // Linha separadora
      sl.addShape(prs.shapes.RECTANGLE, { x:0.5, y:4.4, w:4.0, h:0.015, fill:{color:'FFFFFF',transparency:75}, line:{color:'FFFFFF',width:0} })

      // Rodapé capa
      sl.addText('Prepara Aula', { x:0.5, y:4.58, w:3, h:0.35, fontSize:9.5, color:'FFFFFF', transparency:55, fontFace:'Calibri', margin:0 })
      sl.addText(`${num} / ${numTotal}`, { x:8.8, y:5.2, w:0.9, h:0.28, fontSize:8, color:'FFFFFF', transparency:60, fontFace:'Calibri', align:'right', margin:0 })
      return
    }
    ...
    if (tipo === 'revisao') {
      ...
      addHeader(sl, etapa, num, numTotal)

      sl.addText(trunc(titulo, 60), { x:0.35, y:0.52, w:9.3, h:0.75, fontSize:22, bold:true, color:'0f172a', fontFace:'Calibri', valign:'middle', margin:0 })

      const itens = conteudo.slice(0, 6)
      const cols  = Math.min(itens.length, 3)
      const rows  = Math.ceil(itens.length / cols)
      const cardW = 9.1 / cols - 0.12
      const cardH = rows === 1 ? 2.9 : 1.38

      itens.forEach((item, j) => {
        const col  = j % cols
        const row  = Math.floor(j / cols)
        const cx   = 0.35 + col * (cardW + 0.15)
        const cy   = 1.44 + row * (cardH + 0.12)
        const cor  = circleColors[j % circleColors.length]
        const texto = (item || '').replace(/^\d+\s*[—\-.]\s*/, '').trim()

        sl.addShape(prs.shapes.RECTANGLE, { x:cx, y:cy, w:cardW, h:cardH, fill:{color:cor,transparency:91}, line:{color:cor,transparency:70,width:0.8},
          shadow:{type:'outer',color:'000000',blur:4,offset:1,angle:135,opacity:0.06} })
        sl.addShape(prs.shapes.RECTANGLE, { x:cx, y:cy, w:cardW, h:0.05, fill:{color:cor}, line:{color:cor,width:0} })
        sl.addText(String(j+1).padStart(2,'0'), { x:cx+0.12, y:cy+0.1, w:0.5, h:0.32, fontSize:16, bold:true, color:cor, transparency:25, fontFace:'Calibri', margin:0 })
        sl.addText(trunc(texto, 120), { x:cx+0.12, y:cy+0.4, w:cardW-0.22, h:cardH-0.54, fontSize:10.5, color:'1e293b', fontFace:'Calibri', valign:'top', wrap:true, margin:0 })
      })
    ...
    // ── DADO VISUAL — número enorme ───────────────────────────────────────────
    if (tipo === 'dado_visual') {
      sl.background = { color: 'FFFFFF' }
      addHeader(sl, etapa, num, numTotal)

      sl.addShape(prs.shapes.RECTANGLE, { x:0, y:0.055, w:3.5, h:5.57, fill:{color:C.primary,transparency:91}, line:{color:C.primary,width:0} })

      if (destaque) {
        sl.addText(trunc(destaque, 12), { x:0.15, y:1.0, w:3.2, h:2.3, fontSize:60, bold:true, color:C.primary, fontFace:'Calibri', align:'center', valign:'middle', charSpacing:-2, margin:0 })
        sl.addShape(prs.shapes.RECTANGLE, { x:1.1, y:3.4, w:1.3, h:0.04, fill:{color:C.primary,transparency:50}, line:{color:C.primary,width:0} })
        sl.addText('DADO-CHAVE', { x:0.15, y:3.55, w:3.2, h:0.3, fontSize:7.5, bold:true, color:C.primary, fontFace:'Calibri', align:'center', charSpacing:1.5, margin:0 })
      }

      sl.addText(trunc(titulo, 65), { x:3.8, y:0.55, w:5.9, h:1.0, fontSize:20, bold:true, color:'0f172a', fontFace:'Calibri', valign:'top', wrap:true, margin:0 })
      addBullets(sl, conteudo.slice(0, 6), 3.8, 1.72, 5.9)
      addRodape(sl)
      return
    }

    // ── EXECUÇÃO — passos numerados ───────────────────────────────────────────
    if (tipo === 'execucao') {
      sl.background = { color: 'FFFFFF' }
      addHeader(sl, etapa, num, numTotal)

      sl.addShape(prs.shapes.RECTANGLE, { x:0, y:0.055, w:2.6, h:5.57, fill:{color:C.primary}, line:{color:C.primary,width:0} })
      sl.addShape(prs.shapes.OVAL, { x:0.3, y:-1.0, w:2.5, h:2.5, fill:{color:'FFFFFF',transparency:93}, line:{color:'FFFFFF',width:0} })

      sl.addText('✏️', { x:0.3, y:1.4, w:2.0, h:1.2, fontSize:44, align:'center', valign:'middle', margin:0 })

      if (destaque) sl.addText(trunc(destaque, 25), { x:0.2, y:2.82, w:2.2, h:1.1, fontSize:11, bold:true, color:'FFFFFF', fontFace:'Calibri', align:'center', wrap:true, margin:0 })
      sl.addText('ATIVIDADE', { x:0.2, y:5.0, w:2.2, h:0.35, fontSize:7, bold:true, color:'FFFFFF', fontFace:'Calibri', align:'center', charSpacing:1.5, transparency:50, margin:0 })

      sl.addText(trunc(titulo, 60), { x:2.88, y:0.52, w:6.85, h:0.85, fontSize:21, bold:true, color:'0f172a', fontFace:'Calibri', valign:'top', wrap:true, margin:0 })

      conteudo.slice(0, 6).forEach((item, j) => {
        const cy = 1.52 + j * 0.65
        const cc = circleColors[j % circleColors.length]
        sl.addShape(prs.shapes.ROUNDED_RECTANGLE, { x:2.9, y:cy, w:0.32, h:0.32, fill:{color:cc}, line:{color:cc,width:0}, rectRadius:0.05 })
        sl.addText(String(j+1), { x:2.9, y:cy, w:0.32, h:0.32, fontSize:10, bold:true, color:'FFFFFF', fontFace:'Calibri', align:'center', valign:'middle', margin:0 })
        sl.addText(trunc(item, 100), { x:3.32, y:cy, w:6.38, h:0.55, fontSize:11.5, color: j===0 ? '0f172a' : '374151', fontFace:'Calibri', valign:'middle', wrap:true, margin:0, bold: j===0 })
      })
    ...
    if (tipo === 'introducao') {
      ...
      sl.addShape(prs.shapes.RECTANGLE, { x:5.55, y:0.055, w:4.45, h:5.57, fill:{color:C.primary}, line:{color:C.primary,width:0} })
      sl.addShape(prs.shapes.OVAL, { x:6.6, y:-0.9, w:2.9, h:2.9, fill:{color:'FFFFFF',transparency:92}, line:{color:'FFFFFF',width:0} })
      sl.addShape(prs.shapes.OVAL, { x:5.9, y:3.6,  w:1.5, h:1.5, fill:{color:'FFFFFF',transparency:94}, line:{color:'FFFFFF',width:0} })

      sl.addText(trunc(titulo, 65), { x:0.35, y:0.52, w:5.0, h:1.0, fontSize:20, bold:true, color:'0f172a', fontFace:'Calibri', valign:'top', wrap:true, margin:0 })
      addBullets(sl, conteudo.slice(0, 4), 0.35, 1.7, 5.0)

      sl.addText('PARA PENSAR', { x:5.75, y:0.72, w:4.0, h:0.28, fontSize:7.5, bold:true, color:'FFFFFF', fontFace:'Calibri', charSpacing:1.5, transparency:40, margin:0 })
      conteudo.slice(4, 6).forEach((item, j) => {
        sl.addText(trunc(item, 110), { x:5.75, y:1.1+j*1.05, w:3.95, h:0.95, fontSize: j===0 ? 13.5 : 11, bold: j===0, color:'FFFFFF', transparency: j===0 ? 0 : 25, fontFace:'Calibri', wrap:true, valign:'top', margin:0 })
      })
    ...
    // Coluna esquerda
    sl.addShape(prs.shapes.RECTANGLE, { x:0, y:0.055, w:3.6, h:5.57, fill:{color:C.primary,transparency: isEven ? 91 : 0}, line:{color:C.primary,width:0} })

    // Círculos decorativos
    sl.addShape(prs.shapes.OVAL, { x:0.8, y:0.8, w:2.0, h:2.0, fill:{color: isEven ? C.primary : 'FFFFFF', transparency: isEven ? 86 : 93}, line:{color: isEven ? C.primary : 'FFFFFF', transparency:80, width:0.5} })
    sl.addShape(prs.shapes.OVAL, { x:1.25, y:1.25, w:1.1, h:1.1, fill:{color: isEven ? C.primary : 'FFFFFF', transparency: isEven ? 78 : 90}, line:{color:'FFFFFF',width:0} })

    // Destaque ou número
    if (destaque) {
      sl.addText(trunc(destaque, 14), { x:0.15, y:1.0, w:3.3, h:1.8, fontSize:38, bold:true, color: isEven ? C.primary : 'FFFFFF', fontFace:'Calibri', align:'center', valign:'middle', charSpacing:-1.5, margin:0 })
      sl.addShape(prs.shapes.RECTANGLE, { x:1.1, y:2.94, w:1.4, h:0.038, fill:{color: isEven ? C.primary : 'FFFFFF', transparency:55}, line:{color:C.primary,width:0} })
      sl.addText(tipo === 'exemplo' ? 'CASO REAL' : 'CONCEITO-CHAVE', { x:0.15, y:3.06, w:3.3, h:0.3, fontSize:7, bold:true, color: isEven ? C.primary : 'FFFFFF', fontFace:'Calibri', align:'center', charSpacing:1.2, transparency:40, margin:0 })
    } else {
      sl.addText(num, { x:0.15, y:1.2, w:3.3, h:1.6, fontSize:72, bold:true, color: isEven ? C.primary : 'FFFFFF', transparency:80, fontFace:'Calibri', align:'center', valign:'middle', charSpacing:-3, margin:0 })
    }

    // Emoji tipo + label
    const tipoEmoji = { conceito:'📖', exemplo:'💡', dado_visual:'📊', aplicacao:'⚙️', contexto:'🌍' }
    sl.addText(tipoEmoji[tipo] || '📌', { x:0.3, y:4.55, w:1.0, h:0.65, fontSize:20, align:'center', margin:0 })
    sl.addText((tipo||'CONTEÚDO').toUpperCase(), { x:1.1, y:4.66, w:2.2, h:0.35, fontSize:7, bold:true, color: isEven ? C.primary : 'FFFFFF', fontFace:'Calibri', charSpacing:1, transparency:45, margin:0 })

    // Coluna direita — barra vertical + título
    sl.addShape(prs.shapes.RECTANGLE, { x:3.72, y:0.52, w:0.055, h:0.72, fill:{color:C.primary}, line:{color:C.primary,width:0} })
    sl.addText(trunc(titulo, 70), { x:3.9, y:0.5, w:5.8, h:0.85, fontSize:19, bold:true, color:'0f172a', fontFace:'Calibri', valign:'top', wrap:true, margin:0 })
    sl.addShape(prs.shapes.RECTANGLE, { x:3.72, y:1.48, w:5.8, h:0.008, fill:{color:'e2e8f0'}, line:{color:'e2e8f0',width:0} })

    // Bullets com círculos coloridos
    conteudo.slice(0, 6).forEach((item, j) => {
      const cy = 1.62 + j * 0.64
      const cc = circleColors[j % circleColors.length]
      sl.addShape(prs.shapes.OVAL, { x:3.75, y:cy+0.04, w:0.22, h:0.22, fill:{color:cc}, line:{color:cc,width:0} })
      sl.addText(trunc(item, 110), { x:4.06, y:cy, w:5.6, h:0.56, fontSize:11.5, color: j===0 ? '0f172a' : '374151', fontFace:'Calibri', valign:'middle', wrap:true, margin:0, bold: j===0 })
    })
...
// ─── SLIDE BASE (visualização web) ───────────────────────────────────────────
function SlideBase({ s, p, num, total }) {
  const etapa = ETAPAS[s.etapa]
  const isDark = s.cor_fundo === '#0f172a' || s.cor_fundo === p.dark || s.cor_fundo?.startsWith('#0') || s.cor_fundo === p.primary || s.cor_fundo === '#1e3a5f'
  const tc = s.cor_texto || (isDark ? '#ffffff' : '#0f172a')
  const bg = s.cor_fundo || (isDark ? '#0f172a' : '#ffffff')

  if (s.tipo === 'capa') {
    return (
      <div style={{ width:'100%', height:'100%', background:bg, display:'flex', flexDirection:'column', padding:'44px 56px', position:'relative', overflow:'hidden', fontFamily:"'Plus Jakarta Sans',sans-serif", boxSizing:'border-box' }}>
        ...
        <div style={{ position:'absolute', right:-60, top:-60, width:280, height:280, borderRadius:'50%', border:`1px solid ${p.accent}18` }} />
        <div style={{ position:'absolute', right:20, top:20, width:140, height:140, borderRadius:'50%', border:`1px solid ${p.accent}10` }} />
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'auto' }}>
          {etapa && <div style={{ display:'flex', alignItems:'center', gap:6 }}><div style={{ width:6, height:6, borderRadius:'50%', background:p.accent }} /><span style={{ fontSize:10, fontWeight:700, color:p.accent, letterSpacing:'0.15em', textTransform:'uppercase' }}>{etapa.label}</span></div>}
          <span style={{ fontSize:10, color:`${tc}30`, fontWeight:600, marginLeft:'auto' }}>{String(num).padStart(2,'0')} / {String(total).padStart(2,'0')}</span>
        </div>
        <div style={{ display:'flex', flexDirection:'column', justifyContent:'flex-end', flex:1 }}>
          {s.destaque && <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}><div style={{ width:20, height:2, background:p.accent }} /><span style={{ fontSize:11, fontWeight:700, color:p.accent, letterSpacing:'0.14em', textTransform:'uppercase' }}>{s.destaque}</span></div>}
          <h1 style={{ fontSize:'clamp(2rem,4.5vw,3.8rem)', fontWeight:900, color:tc, lineHeight:1.05, letterSpacing:'-0.03em', margin:'0 0 16px', maxWidth:'75%' }}>{s.titulo}</h1>
          {s.subtitulo && <p style={{ fontSize:'clamp(0.85rem,1.4vw,1.15rem)', color:`${tc}70`, lineHeight:1.6, margin:'0 0 24px', maxWidth:480 }}>{s.subtitulo}</p>}
          {s.conteudo?.[0] && (
            <div style={{ display:'inline-flex', alignItems:'center', gap:10, padding:'10px 16px', background:`${p.accent}12`, border:`1px solid ${p.accent}25`, borderRadius:8, width:'fit-content' }}>
  ...
  if (s.tipo === 'introducao') {
    ...
    return (
      <div style={{ width:'100%', height:'100%', display:'grid', gridTemplateColumns:'55% 45%', fontFamily:"'Plus Jakarta Sans',sans-serif", overflow:'hidden' }}>
        <div style={{ background:bg, padding:'36px 40px 36px 56px', display:'flex', flexDirection:'column', justifyContent:'center', borderRight:`1px solid ${tc}08`, boxSizing:'border-box' }}>
          <SlideHeader s={s} p={p} num={num} total={total} tc={tc} />
          <h2 style={{ fontSize:'clamp(1.2rem,2.2vw,1.9rem)', fontWeight:900, color:tc, lineHeight:1.15, letterSpacing:'-0.02em', margin:'0 0 20px' }}>{s.titulo}</h2>
          <div style={{ display:'flex', flexDirection:'column', gap:9 }}>{[l1,l2,l3].filter(Boolean).map((l,i)=><BulletItem key={i} text={l} tc={tc} p={p} size={13} />)}</div>
        </div>
        <div style={{ background:p.primary, display:'flex', flexDirection:'column', justifyContent:'center', padding:'36px 40px', position:'relative', overflow:'hidden', boxSizing:'border-box' }}>
          <div style={{ position:'absolute', top:-40, right:-40, width:160, height:160, borderRadius:'50%', background:'rgba(255,255,255,0.06)' }} />
          <div style={{ position:'absolute', bottom:-20, left:-20, width:100, height:100, borderRadius:'50%', background:'rgba(255,255,255,0.04)' }} />
          <div style={{ fontSize:10, color:'rgba(255,255,255,0.45)', fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', marginBottom:12 }}>Para pensar</div>
          {[l4,l5].filter(Boolean).map((l,i)=>(
            <p key={i} style={{ fontSize: i===0?'clamp(0.9rem,1.6vw,1.2rem)':13, color: i===0?'#fff':'rgba(255,255,255,0.6)', fontWeight: i===0?700:400, lineHeight:1.55, margin:'0 0 12px', position:'relative', zIndex:1 }}>{l}</p>
          ))}
          {s.destaque && <div style={{ marginTop:16, padding:'8px 14px', background:'rgba(255,255,255,0.1)', borderRadius:7, fontSize:12, color:'rgba(255,255,255,0.8)', fontWeight:600, display:'inline-block', width:'fit-content' }}>{s.destaque}</div>}
        </div>
      </div>
    )
  }
  ...
  if (s.tipo === 'dado_visual') {
    return (
      <div style={{ width:'100%', height:'100%', background:bg, padding:'44px 56px', display:'flex', flexDirection:'column', fontFamily:"'Plus Jakarta Sans',sans-serif", boxSizing:'border-box', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:380, height:380, borderRadius:'50%', border:`1px solid ${tc}12` }} />
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:240, height:240, borderRadius:'50%', border:`1px solid ${tc}08` }} />
        <SlideHeader s={s} p={p} num={num} total={total} tc={tc} />
        <div style={{ flex:1, display:'flex', gap:40, alignItems:'center' }}>
          <div style={{ textAlign:'center', minWidth:200, position:'relative', zIndex:1 }}>
            <div style={{ fontSize:'clamp(3rem,8vw,6.5rem)', fontWeight:900, color:tc, lineHeight:0.9, letterSpacing:'-0.05em', marginBottom:8 }}>{s.destaque||'—'}</div>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', gap:10 }}>
            ...
            {s.conteudo?.map((l,i)=><BulletItem key={i} text={l} tc={tc} p={p} size={13} />)}
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div style={{ width:'100%', height:'100%', background:bg, padding:'44px 56px', display:'flex', flexDirection:'column', fontFamily:"'Plus Jakarta Sans',sans-serif", boxSizing:'border-box', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', gap:36, flex:1, alignItems:'flex-start', marginTop:8 }}>
          <div style={{ minWidth:110, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:`${p.primary}15`, borderRadius:12, padding:'20px 14px', alignSelf:'stretch' }}>
            <div style={{ fontSize:32, marginBottom:8 }}>✏️</div>
            {s.destaque && <div style={{ fontSize:11, fontWeight:800, color:p.primary, textAlign:'center', lineHeight:1.3 }}>{s.destaque}</div>}
          </div>
          <div style={{ flex:1 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {s.conteudo?.map((l,i)=>(
                <div key={i} style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
                  <div style={{ minWidth:22, height:22, borderRadius:6, background:p.primary, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:800, color:'#fff', flexShrink:0, marginTop:2 }}>{i+1}</div>
  ...
  if (s.tipo === 'revisao') {
    return (
      <div style={{ width:'100%', height:'100%', background:bg, padding:'44px 56px', display:'flex', flexDirection:'column', fontFamily:"'Plus Jakarta Sans',sans-serif", boxSizing:'border-box' }}>
        <SlideHeader s={s} p={p} num={num} total={total} tc={tc} />
        <h2 style={{ fontSize:'clamp(1.3rem,2.4vw,2rem)', fontWeight:900, color:tc, letterSpacing:'-0.02em', margin:'0 0 20px' }}>{s.titulo}</h2>
        <div style={{ display:'grid', gridTemplateColumns:`repeat(${Math.min(s.conteudo?.length||3,3)},1fr)`, gap:12, flex:1 }}>
          {s.conteudo?.map((l,i)=>{
            const texto = l.replace(/^\d+\s*[—\-.]\s*/,'').trim()
            const isQ = texto.includes('?')
            return (
              <div key={i} style={{ background: isQ?p.primary:`${p.primary}0a`, borderRadius:10, padding:'16px 14px', border: isQ?'none':`1px solid ${p.primary}18`, borderTop: isQ?'none':`2px solid ${p.primary}`, display:'flex', flexDirection:'column' }}>
                {!isQ && <div style={{ fontSize:18, fontWeight:900, color:`${p.primary}25`, lineHeight:1, marginBottom:8 }}>{String(i+1).padStart(2,'0')}</div>}
  ...
  return (
    <div style={{ width:'100%', height:'100%', background:bg, display:'grid', gridTemplateColumns:'38% 62%', fontFamily:"'Plus Jakarta Sans',sans-serif", overflow:'hidden' }}>
      <div style={{ background:accentBg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'36px 28px', position:'relative', overflow:'hidden', boxSizing:'border-box' }}>
        ...
        <div style={{ textAlign:'center', position:'relative', zIndex:1 }}>
          {s.destaque ? (
            <>
              <div style={{ fontSize:'clamp(1.6rem,4vw,3rem)', fontWeight:900, color:accentText, lineHeight:0.95, letterSpacing:'-0.04em', marginBottom:8 }}>{s.destaque}</div>
              <div style={{ width:24, height:2, background:`${accentText}50`, margin:'0 auto 6px' }} />
              <div style={{ fontSize:9, color:`${accentText}60`, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase' }}>{s.tipo==='exemplo'?'caso real':'conceito-chave'}</div>
            </>
          ) : (
            <div style={{ fontSize:'clamp(2rem,5vw,3.8rem)', fontWeight:900, color:`${accentText}20`, lineHeight:0.9, letterSpacing:'-0.05em' }}>{String(num).padStart(2,'0')}</div>
          )}
        </div>
      </div>
      <div style={{ padding:'36px 40px 36px 32px', display:'flex', flexDirection:'column', justifyContent:'center', boxSizing:'border-box', overflow:'hidden' }}>
        ...
        <h2 style={{ fontSize:'clamp(1rem,1.9vw,1.6rem)', fontWeight:900, color:tc, lineHeight:1.15, letterSpacing:'-0.02em', margin:'0 0 18px' }}>{s.titulo}</h2>
        <div style={{ display:'flex', flexDirection:'column', gap:9, overflow:'hidden' }}>
          {s.conteudo?.slice(0,6).map((l,i)=><BulletItem key={i} text={l} tc={tc} p={p} size={12.5} dim={i>0} />)}
        </div>
      </div>
    </div>
  )
...
function SlideHeader({ s, p, num, total, tc, rightAligned }) {
  const etapa = ETAPAS[s.etapa]
  return (
    <div style={{ display:'flex', justifyContent: rightAligned?'flex-end':'space-between', alignItems:'center', marginBottom:14, flexShrink:0 }}>
      {!rightAligned && etapa && <div style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'3px 9px', background:etapa.bg, borderRadius:100, fontSize:9, fontWeight:700, color:etapa.cor, letterSpacing:'0.1em', textTransform:'uppercase' }}>{etapa.label}</div>}
      {rightAligned && etapa && <div style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'3px 9px', background:etapa.bg, borderRadius:100, fontSize:9, fontWeight:700, color:etapa.cor, letterSpacing:'0.1em', textTransform:'uppercase', marginRight:10 }}>{etapa.label}</div>}
      <span style={{ fontSize:9, color:`${tc}25`, fontWeight:600 }}>{String(num).padStart(2,'0')} / {String(total).padStart(2,'0')}</span>
    </div>
  )
}
...
function BulletItem({ text, tc, p, size=13, dim=false }) {
  return (
    <div style={{ display:'flex', gap:9, alignItems:'flex-start' }}>
      <div style={{ width:5, height:5, borderRadius:'50%', background:p.primary, marginTop:(size*1.65)/2-2.5, flexShrink:0 }} />
      <p style={{ fontSize:size, color:`${tc}${dim?'bb':'ee'}`, lineHeight:1.65, margin:0, overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' }}>{text}</p>
    </div>
  )
}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
            {atividade.exercicios?.map(ex=>(
              <div key={ex.numero} style={{ borderBottom:'1px solid #f1f5f9', paddingBottom:18 }}>
                ...
                {!ex.alternativas && <div style={{ padding:'10px 12px', background:'#f8fafc', borderRadius:7, border:'1px solid #e2e8f0', fontSize:12, color:'#94a3b8', fontStyle:'italic' }}>Resposta dissertativa</div>}
              </div>
            ))}
          </div>
        )}
  ...
  if (modoGrade) return (
    <div style={{ minHeight:'100vh', background:'#0f172a', fontFamily:"'Plus Jakarta Sans',sans-serif", padding:'64px 24px 24px' }}>
      <div style={{ position:'fixed', top:0, left:0, right:0, height:52, background:'rgba(15,23,42,0.97)', backdropFilter:'blur(8px)', borderBottom:'1px solid rgba(255,255,255,0.07)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 20px', zIndex:10 }}>
        ...
        <button onClick={()=>setModoGrade(false)} style={{ background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.12)', color:'white', padding:'5px 14px', borderRadius:7, cursor:'pointer', fontSize:12, fontWeight:600, fontFamily:'inherit' }}>Fechar grade</button>
      </div>
    ...
    <div style={{ minHeight:'100vh', background:'#0b1120', fontFamily:"'Plus Jakarta Sans',sans-serif", display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ position:'fixed', top:0, left:0, right:0, height:50, background:'rgba(11,17,32,0.98)', backdropFilter:'blur(12px)', borderBottom:'1px solid rgba(255,255,255,0.07)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px', zIndex:100 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <button onClick={()=>setIdx(i=>Math.max(i-1,0))} disabled={idx===0} style={{ width:28, height:28, borderRadius:6, border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'center', cursor:idx===0?'not-allowed':'pointer', opacity:idx===0?0.3:1, color:'rgba(255,255,255,0.8)' }}><Prev /></button>
          <span style={{ color:'rgba(255,255,255,0.55)', fontSize:12, fontWeight:700, minWidth:52, textAlign:'center' }}>{idx+1} <span style={{ opacity:.3 }}>/</span> {slides.length}</span>
          <button onClick={()=>setIdx(i=>Math.min(i+1,slides.length-1))} disabled={idx===slides.length-1} style={{ width:28, height:28, borderRadius:6, border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'center', cursor:idx===slides.length-1?'not-allowed':'pointer', opacity:idx===slides.length-1?0.3:1, color:'rgba(255,255,255,0.8)' }}><Next /></button>
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:5 }}>
          ...
          <button onClick={handleExportarPPT} disabled={exportando} style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 13px', background: exportando?'#64748b':p.primary, border:'none', borderRadius:7, color:'white', cursor: exportando?'wait':'pointer', fontSize:12, fontWeight:700, fontFamily:'inherit', boxShadow: exportando?'none':`0 2px 10px ${p.primary}55`, transition:'all 0.15s' }}>
